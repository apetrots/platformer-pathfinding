...
// Add a jump node
int64_t PlatformerAStar2D::add_jump_node(int64_t from, int64_t to, real_t duration) {
    int64_t jump_node = get_point_count();

    // this->add_point(jump_node, get_point_position(from), jump_node_weight_scale);
    this->add_point(jump_node, Vector2(INFINITY, INFINITY), jump_node_weight_scale);
...
const JumpInfo* PlatformerAStar2D::get_jump_info(int64_t node_id) {
    if (abs(this->get_point_weight_scale(node_id) - jump_node_weight_scale) > 0.01f)
        return NULL;

    auto it = jumps.find(node_id);
    if (it != jumps.end()) {
        return &it->second;
    }

    return NULL; // Return a default Vector2 if not found
}